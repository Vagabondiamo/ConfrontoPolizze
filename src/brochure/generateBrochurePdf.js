...
function drawPolicyPanel(doc, x0, w, p, colors, opts) {
  ...
  p.coverages.forEach((c) => {
    ...
    fill(doc, p.accent);
    // quadratino accento (come il preview usa span quadrato)
  ...
  const drawExclusions = () => {
    ...
    wrapped.forEach((ln) => {
      ...
      fill(doc, colors.dot);
      // cerchietto rosso come nel preview
  ...
  // DA VERIFICARE — stile con bordo sinistro rosso come nel preview (border-l-2 border-red-500)
  if (p.datiMancanti && p.datiMancanti.filter((x) => x && x.trim()).length > 0) {
    ...
    // sfondo rosso chiaro (bg-red-50)
    doc.setFillColor(255, 241, 242);
    doc.setDrawColor(239, 68, 68);
    doc.setLineWidth(0.25);
    doc.roundedRect(innerX, startY, innerW, h, 1.2, 1.2, "FD");
    // bordo sinistro spesso (border-l-2) — identico al preview
    doc.setFillColor(239, 68, 68);
    doc.rect(mirror ? innerX + innerW - 2 : innerX, startY, 2, h, "F");
    doc.setLineWidth(0.2);
...
function drawCenterPanel(doc, x0, w, c, colors) {
  ...
  // Descrizione (text-left come nel preview)
  text(doc, cc.text);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.6);
...
/**
 * Genera la brochure PDF usando jsPDF (veloce, senza html2canvas).
 * @returns {Blob} Il PDF come Blob
 */
export function generateBrochurePdf(rawData) {
