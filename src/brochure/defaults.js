...
// Unisce i dati inseriti con gli esempi (solo centro); le polizze non hanno fallback.
export function resolveData(data) {
  const e = EXAMPLE;
  return {
    themeId: data.themeId || e.themeId,
    policyA: {
      name: data.policyA.name || "",
      company: data.policyA.company || "",
      docType: data.policyA.docType || "",
      capitaleDesc: data.policyA.capitaleDesc || "",
      capitaleMin: data.policyA.capitaleMin || "",
      durataDesc: data.policyA.durataDesc || "",
      durataMax: data.policyA.durataMax || "",
      fiscalita: data.policyA.fiscalita || "",
      recesso: data.policyA.recesso || "",
      riscatto: data.policyA.riscatto || "",
      coverages: (data.policyA.coverages || []).filter((x) => x && x.trim()),
      exclusions: (data.policyA.exclusions || []).filter((x) => x && x.trim()),
      note: (data.policyA.note || []).filter((x) => x && x.trim()),
    },
    center: {
    ...
    policyB: {
      name: data.policyB.name || "",
      company: data.policyB.company || "",
      docType: data.policyB.docType || "",
      capitaleDesc: data.policyB.capitaleDesc || "",
      capitaleMin: data.policyB.capitaleMin || "",
      durataDesc: data.policyB.durataDesc || "",
      durataMax: data.policyB.durataMax || "",
      fiscalita: data.policyB.fiscalita || "",
      recesso: data.policyB.recesso || "",
      riscatto: data.policyB.riscatto || "",
      coverages: (data.policyB.coverages || []).filter((x) => x && x.trim()),
      exclusions: (data.policyB.exclusions || []).filter((x) => x && x.trim()),
      note: (data.policyB.note || []).filter((x) => x && x.trim()),
    },
  };
}
